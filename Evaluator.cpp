...
float Evaluator::evaluateFactorial(float n) { return tgamma(n + 1); }
...
float Evaluator::evaluate() { return evaluate_(mExpressionParser.getRoot()); }

float Evaluator::evaluate_(Node *base) {
    if(!base) throw std::runtime_error("Node is null in evaluate_()");
    if(base->token.mType == TokenType::Number) return std::stof(base->token.mValue);
